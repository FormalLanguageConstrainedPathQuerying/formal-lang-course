...
def cfg_to_weak_normal_form(cfg: pyformlang.cfg.CFG) -> pyformlang.cfg.CFG:
    grammatical = cfg.eliminate_unit_productions().remove_useless_symbols()
    _new_productions = grammatical._decompose_productions(
        grammatical._get_productions_with_only_single_terminals()
    )
    return CFG(start_symbol=grammatical.start_symbol, productions=_new_productions)
...
def cfpq_with_hellings(
    ...
    for _production in grammatics.productions:
        if len(_production.body) == 1 and isinstance(_production.body[0], Terminal):
            p_1.setdefault(_production.head, set()).add(_production.body[0])
        elif len(_production.body) == 1 and isinstance(_production.body[0], Epsilon):
            p_2.add(_production.body[0])
        elif len(_production.body) == 2:
            (
                p_3.setdefault(_production.head, set()).add(
                    (_production.body[0], _production.body[1])
                )
            )
    ...
    answer = {
        ...
        if v_i in start_nodes
        and u_i in final_nodes
        and Variable(n_i) == cfg.start_symbol
    }

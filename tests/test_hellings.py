...
from project.querying import query_to_graph_with_hellings
...
def _check_querying(text: str, graph: MultiDiGraph, expected: set):
    actual = query_to_graph_with_hellings(graph, CFG.from_text(text))
    assert actual == expected


def test_1_empty_hellings():
    graph = GraphUtils.create_two_cycle_labeled_graph(1, 2, ("a", "b"))
    _check_querying(
        "\n".join(["S -> B A B ", "A -> a A a | a", "B -> b"]), graph, set()
    )
    print("test_1_empty_hellings test asserted")


def test_2_simple_hellings():
    graph = GraphUtils.create_two_cycle_labeled_graph(1, 2, ("a", "b"))
    ...
    print("test_2_simple_hellings test asserted")


def test_3_big_query_hellings():
    graph = GraphUtils.create_two_cycle_labeled_graph(1, 2, ("a", "b"))
    _check_querying(
    ...
    print("test_3_big_query_hellings test asserted")


def test_4_constrained_empty_hellings():
...
def test_5_constrained_hellings():
    ...
    assert actual == {(3, 2)}
    print("test_4_constrained_hellings test asserted")

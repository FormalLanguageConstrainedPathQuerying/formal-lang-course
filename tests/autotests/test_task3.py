...
try:
    from project.task3 import intersect_automata, paths_ends, FiniteAutomaton
...
class TestIntersect:
    @pytest.mark.parametrize(
        ...
        REGEX_TO_TEST,
        ids=lambda regex_tuple: (regex_tuple[0], regex_tuple[1]),
    )
    def test(self, regex_str1: str, regex_str2: str) -> None:
        ...
        # I don`t know why, but intersection works only with regex or FA. Not with CFG
        intersect_cfg: pyformlang.cfg.CFG = cfg_of_regex1.intersection(regex2)
        words = intersect_cfg.get_words(max_length=10**2)
        word = map(lambda x: x.value, random.choice(words))

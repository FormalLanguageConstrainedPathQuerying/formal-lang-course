...
def test_pyformlang_intersect_default(lhs_nfa, rhs_nfa):
    ...
    lhs_bm = RSMMatrixSparse.from_nfa(lhs_nfa)
    rhs_bm = RSMMatrixSparse.from_nfa(rhs_nfa)
...
def test_pyformlang_intersect_with_several(nfa_several_labels, rhs_nfa):
    ...
    lhs_bm = RSMMatrixSparse.from_nfa(nfa_several_labels)
    rhs_bm = RSMMatrixSparse.from_nfa(rhs_nfa)
...
def test_intersect_with_empty(lhs_nfa, empty_nfa):
    ...
    lhs_bm = RSMMatrixSparse.from_nfa(lhs_nfa)
    rhs_bm = RSMMatrixSparse.from_nfa(empty_nfa)
...
def test_renumbered_keys(nfa_unordered_states):
    nfa_bm = RSMMatrixSparse.from_nfa(nfa_unordered_states)

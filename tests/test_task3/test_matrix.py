...
def test_transitive_closure(actual: BooleanDecomposition, expected):
    if len(expected) == 0:
        ...
        assert (
            actual.make_transitive_closure()
            .toarray()
            .data.__eq__(dok_matrix(expected).toarray().data)
        )
...
@pytest.mark.parametrize(
    ...
    get_data(
        "test_to_automaton",
        lambda data: (create_nfa_from_graph(dot_to_graph(data["expected"]))),
    ),
)
...
def test_init(decomposition: BooleanDecomposition, expected, starts, finals):
    ...
    for (letter, values) in decomposition.bool_decomposition.items():
        isMatriciesCorrect = isMatriciesCorrect and eq_matrix(
            expected[letter], values.toarray()
        )
